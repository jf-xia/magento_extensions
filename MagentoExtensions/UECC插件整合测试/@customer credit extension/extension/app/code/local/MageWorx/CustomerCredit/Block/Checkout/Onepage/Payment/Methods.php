...
class MageWorx_CustomerCredit_Block_Checkout_Onepage_Payment_Methods extends Mage_Checkout_Block_Onepage_Payment_Methods {
    ...
    public function useCheckbox($method){
        ...
        if(Mage::getSingleton('checkout/session')->getUseInternalCredit() && $this->getQuote()->getBaseCustomerCreditTotal() > $this->getCreditValue()){
            return true;
        }

        if($this->getQuote()->getBaseGrandTotal() > $this->getCreditValue()){
            return true;
        }
