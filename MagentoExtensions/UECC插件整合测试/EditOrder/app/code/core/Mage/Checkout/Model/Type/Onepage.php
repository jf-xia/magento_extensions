...
class Mage_Checkout_Model_Type_Onepage
{
    ...
    public function saveOrder()
    {
        ...
        //if (($this->getQuote()->getEntityId()!=null)) {
        	...
        	if ($editOrder!=null) {
	        	...
	        	if ($oldOrder!=null) {
	        		...
	        		$totalDue=$this->getQuote()->getSubtotalWithDiscount();
	        		$totalPaid=$oldOrder->getTotalPaid();
	        		$moneyBack=$totalPaid-$totalDue;
	        		...
	        		$orderData = array(
	        				...
	        				'total_paid'                => $totalPaid,//Total Paid
	        				'base_total_invoiced_cost'  => $oldOrder->getBaseTotalInvoicedCost()//Base Total Invoiced Cost
	        		);
	        		$this->getQuote()->setReservedOrderId($orderData['increment_id']);
        		...
        		if ($moneyBack>=0) {
        			...
        			//$totalPaid=$totalDue;
        			//Mage::getSingleton('customer/session')->setPaidOrder("paid");
        			//$moneyBack=1;
        			Mage::getModel('customercredit/credit')
        			->setValueChange($moneyBack)
        			//->setOrder($order)
        			->setCustomerId($this->getQuote()->getCustomerId())
        			->setWebsiteId(Mage::app()->getStore($this->getQuote()->getStoreId())->getWebsiteId())
        			->save();
        		}
        	}
        	//if ($oldOrder->getStatus()=="canceled"||$oldOrder==null) {
        $service->submitAll();
        	//}
