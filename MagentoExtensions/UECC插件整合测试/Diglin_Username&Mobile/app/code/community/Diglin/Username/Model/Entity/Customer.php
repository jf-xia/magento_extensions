...
class Diglin_Username_Model_Entity_Customer extends Mage_Customer_Model_Entity_Customer{
    ...
    protected function _beforeSave(Varien_Object $customer)
    {
        ...
        $resultMobile = $model->customerMobileExists($customer->getMobile(), $websiteId);
        if ($resultMobile && $resultMobile->getId() != $customer->getId()) {
            throw Mage::exception('Mage_Core', Mage::helper('customer')->__("Mobile already exists"));
        }
    ...
    public function loadByMobile(Mage_Customer_Model_Customer $customer, $mobile, $testOnly = false)
    {
        $select = $this->_getReadAdapter()->select()
            ...
            ->joinNatural(array('cev' => $this->getTable('customer_entity_varchar')))
            ->joinNatural(array('ea' => $this->getTable('eav_attribute')))
            ->where('ea.attribute_code=\'mobile\' AND cev.value=?',$mobile);
        if ($customer->getSharingConfig()->isWebsiteScope()) {
            if (!$customer->hasData('website_id')) {
                Mage::throwException(Mage::helper('customer')->__('Customer website ID must be specified when using the website scope.'));
            }
