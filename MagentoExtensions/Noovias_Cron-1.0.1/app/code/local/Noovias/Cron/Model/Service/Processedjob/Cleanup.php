<?php
/**
 * Noovias_Cron_Model_Service_Processedjob_Cleanup
 *
 * NOTICE OF LICENSE
 *
 ...
 */
class Noovias_Cron_Model_Service_Processedjob_Cleanup extends Noovias_Cron_Model_Service_Abstract
{
    /**
     * @param $scheduleId
     */
    public function executeByScheduleId($scheduleId)
    {
        $modelProcessed = $this->getFactory()->getModelProcessedjob();

        $collection = $modelProcessed->getCollection();
        /** @var $collection Noovias_Cron_Model_Mysql4_Processedjob_Collection */
        $collection->addFieldToFilter('schedule_id', $scheduleId);

        foreach ($collection->getItems() as /** @var $item Noovias_Cron_Model_Processedjob */$item) {
            $item->delete();
        }
    }
}