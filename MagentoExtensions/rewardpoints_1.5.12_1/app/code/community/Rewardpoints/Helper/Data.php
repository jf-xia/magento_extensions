...
class Rewardpoints_Helper_Data extends Mage_Core_Helper_Abstract {
    ...
    public function getProductPointsText($product, $noCeil = false, $from_list = false){
        ...
        return '<p class="j2t-loyalty-points inline-points" style="display:none;">'.$img. Mage::helper('rewardpoints')->__("With this product, you earn <span id='j2t-pts'>%d</span> loyalty point(s).", $points) . '</p>';
    }
    ...
    public function getPointsOnOrder($cartLoaded = null, $cartQuote = null, $specific_rate = null, $exclude_rules = false, $storeId = false){
        ...
        foreach ($items as $_item){
            ...
            if ($_item->getHasChildren()){
                ...
                //echo $bundle_price;
                $rewardPoints += (int)(Mage::getStoreConfig('rewardpoints/default/money_points', $storeId) * $bundle_price);
                continue;
            }
            ...
            if ($product_points > 0){
                ...
                if (!$_item->getParentItemId()) {
                    ...
                    /*$current_item = $_item;  
                    if ($_item->getParentItemId() && $_item->getParentItem()->getProduct()->getTypeId() == Mage_Catalog_Model_Product_Type::TYPE_CONFIGURABLE) {
