 ...
 */
class CKApps_Phonenumber_Model_Entity_Customer extends Mage_Customer_Model_Entity_Customer{
    ...
    protected function _beforeSave(Varien_Object $customer)
    {
        ...
        $result = $model->customerPhonenumberExists($customer->getPhonenumber(), $websiteId);
        if ($result && $result->getId() != $customer->getId()) {
            throw Mage::exception('Mage_Core', Mage::helper('customer')->__("Phonenumber already exists"));
        }
     ...
     */
    public function loadByPhonenumber(Mage_Customer_Model_Customer $customer, $phonenumber, $testOnly = false)
    {
        $select = $this->_getReadAdapter()->select()
            ...
            ->where('ea.attribute_code=\'phonenumber\' AND cev.value=?',$phonenumber);
        if ($customer->getSharingConfig()->isWebsiteScope()) {
