...
class Rewardpoints_IndexController extends Mage_Core_Controller_Front_Action
{
    ...
    public function changegroupAction()
    {
        ...
        $reward_model = Mage::getModel('rewardpoints/stats')->getPointsCurrent($customerId, $storeId);
		if($customer->getGroupId()==$newGroupId) {
    		$messageVIP = $this->__('Sign Up As VIP Error, you are already VIP user!');
    		Mage::getSingleton('customer/session')->addError($messageVIP);
    		session_write_close();
    		$this->_redirect('customer/account');
		}
		if(($reward_model>=$cost)&&($customer->getGroupId()!=$newGroupId)) {
			$model = Mage::getModel('rewardpoints/stats');
			$model->setPointsSpent($cost);
			$model->setCustomerId($customerId); 
			$model->setStoreId($storeId);
			$model->setOrderId(Rewardpoints_Model_Stats::TYPE_POINTS_REVIEW); 
			$model->save();
			$customer->setGroupId($newGroupId)->save(); 
    		$messageSuccess = $this->__('Sign Up As VIP Success!');
    		Mage::getSingleton('customer/session')->addSuccess($messageSuccess);
    		session_write_close();
    		$this->_redirect('customer/account');
    	}
    	else {
    		$messageError = $this->__('Sign Up As VIP Error, not enough Points!');
