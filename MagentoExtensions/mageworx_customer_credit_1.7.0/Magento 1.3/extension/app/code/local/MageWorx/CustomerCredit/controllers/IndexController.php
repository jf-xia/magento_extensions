...
class MageWorx_CustomerCredit_IndexController extends Mage_Core_Controller_Front_Action
{
    ...
    public function logAction()
    {
        ...
        $this->_initLayoutMessages('customer/session');
        $this->loadLayoutUpdates();
        $this->getLayout()->getBlock('head')->setTitle('My Credit Activity');
	    ...
	    if ($this->getRequest()->has('customercredit_code'))
	    {
	        ...
	        try
	        {
                ...
                Mage::getSingleton('customer/session')->addSuccess($this->__(
                    'Credit Balance was refilled with %s successfully using Recharge Code "%s".',
                    Mage::helper('core')->currency($refillCredit),
                    Mage::helper('core')->htmlEscape($codeModel->getCode()))
                );
	        }
