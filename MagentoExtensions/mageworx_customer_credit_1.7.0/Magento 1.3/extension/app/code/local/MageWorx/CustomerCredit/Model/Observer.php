...
class MageWorx_CustomerCredit_Model_Observer
{
    ...
    public function saveCreditmemoAfter(Varien_Event_Observer $observer)
    {
        ...
        if ($creditmemo->getBaseCustomerCreditAmount())
        {
            ...
            Mage::getModel('customercredit/credit')->refund($creditmemo);
        }
        return $this;
    }
    	...
    	if($customerId = Mage::getSingleton('customer/session')->getCustomerId()){
	    	...
	    	foreach ($ruleModel->getData() as $rule){
	    		...
	    		foreach ($conditions['conditions'] as $key => $condition){
		    		...
		    		if($condition['attribute'] == 'registration'){
		    			...
		    			$orders->getSelect()
		    					...
		    					->join(array('order_varchar' => 'sales_order_varchar'),
		    								'order_varchar.entity_type_id=e.entity_type_id
												AND order_varchar.entity_id=e.entity_id
												AND order_varchar.attribute_id="215"
												AND order_varchar.value="complete"',
		    								array('grand_subtotal' => 'SUM(e.subtotal)'))
		    					->where('customer_id='.$customerId.' AND grand_total>0')
		    					->group('customer_id');
	    		...
	    		if ($result){
		    			...
		    			$data = array('rule_id' => $rule['rule_id'], 'customer_id' => $customerId);
		    			Mage::getModel('customercredit/rules_customer')->setData('rule_id', $rule['rule_id'])
