...
class Rewardpoints_Block_Dashboard extends Mage_Core_Block_Template
{
    ...
    public function getPointsReceived(){
        ...
        if (Mage::getStoreConfig('rewardpoints/default/flatstats', $store_id)){
            $reward_flat_model = Mage::getModel('rewardpoints/flatstats');
            return $reward_flat_model->collectPointsReceived($customerId, $store_id);
        }
        
        $reward_model = Mage::getModel('rewardpoints/stats');
        
        //return $reward_model->getPointsReceived($customerId, $store_id);
        return $reward_model->getRealPointsReceivedNoExpary($customerId, $store_id);
    }
    ...
    public function getPointsSpent(){
        ...
        if (Mage::getStoreConfig('rewardpoints/default/flatstats', $store_id)){
            $reward_flat_model = Mage::getModel('rewardpoints/flatstats');
            return $reward_flat_model->collectPointsSpent($customerId, $store_id);
        }
    ...
    public function getPointsWaitingValidation(){
        ...
        if (Mage::getStoreConfig('rewardpoints/default/flatstats', $store_id)){
            $reward_flat_model = Mage::getModel('rewardpoints/flatstats');
            return $reward_flat_model->collectPointsWaitingValidation($customerId, $store_id);
        }
    ...
    public function getPointsLost() {
        ...
        if (Mage::getStoreConfig('rewardpoints/default/flatstats', $store_id)){
            $reward_flat_model = Mage::getModel('rewardpoints/flatstats');
            return $reward_flat_model->collectPointsLost($customerId, $store_id);
        }        
        
        $reward_model = Mage::getModel('rewardpoints/stats');
        return $reward_model->getRealPointsLost($customerId, $store_id);
    }
