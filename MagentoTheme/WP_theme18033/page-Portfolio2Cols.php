...
<ul class="portfolio">
	...
	?>
	<li class="<?php echo $addclass; ?> folio_2 ">
        <div class="folioThumbnail">
    		<?php if($lightbox!=""){ ?>
    			<a class="thumbnail" href="<?php echo $lightbox;?>" rel="fancybox" title="<?php the_title();?>"><?php the_post_thumbnail( 'portfolio-post-thumbnail-large' ); ?><span class="zoom-icon"></span></a>
    		<?php }else{ ?>
    			<a class="thumbnail" href="<?php the_permalink() ?>" title="<?php _e('Permanent Link to', 'theme_18033');?> <?php the_title_attribute(); ?>" ><?php the_post_thumbnail( 'portfolio-post-thumbnail-large' );?></a>
    		<?php } ?>
