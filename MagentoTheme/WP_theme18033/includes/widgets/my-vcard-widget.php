...
class MY_Vcard_Widget extends WP_Widget {
  function MY_Vcard_Widget() {
    $widget_ops = array('classname' => 'widget_my_framework_vcard', 'description' => __('Use this widget to add a vCard', 'theme_5819'));
    $this->WP_Widget('widget_my_framework_vcard', __('My - vCard', 'theme_5819'), $widget_ops);
    $this->alt_option_name = 'widget_my_framework_vcard';
  ...
  function widget($args, $instance) {
    $cache = wp_cache_get('widget_my_framework_vcard', 'widget');
    ...
    $title = apply_filters('widget_title', empty($instance['title']) ? __('vCard', 'theme_5819') : $instance['title'], $instance, $this->id_base);
  ...
  <?php
    ...
    wp_cache_set('widget_my_framework_vcard', $cache, 'widget');
  }
  ...
  function update($new_instance, $old_instance) {
    ...
    if (isset($alloptions['widget_my_framework_vcard'])) {
      delete_option('widget_my_framework_vcard');
    }
  ...
  function flush_widget_cache() {
    wp_cache_delete('widget_my_framework_vcard', 'widget');
  }
  ...
  ?>
    <p>
      <label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php _e('Title (optional):', 'theme_5819'); ?></label>
      <input class="widefat" id="<?php echo esc_attr($this->get_field_id('title')); ?>" name="<?php echo esc_attr($this->get_field_name('title')); ?>" type="text" value="<?php echo esc_attr($title); ?>" />
    </p>
    <p>
      <label for="<?php echo esc_attr($this->get_field_id('street_address')); ?>"><?php _e('Street Address:', 'theme_5819'); ?></label>
    ...
    <p>
      <label for="<?php echo esc_attr($this->get_field_id('locality')); ?>"><?php _e('City/Locality:', 'theme_5819'); ?></label>
    ...
    <p>
      <label for="<?php echo esc_attr($this->get_field_id('region')); ?>"><?php _e('State/Region:', 'theme_5819'); ?></label>
    ...
    <p>
      <label for="<?php echo esc_attr($this->get_field_id('postal_code')); ?>"><?php _e('Zipcode/Postal Code:', 'theme_5819'); ?></label>
    ...
    <p>
      <label for="<?php echo esc_attr($this->get_field_id('tel')); ?>"><?php _e('Telephone:', 'theme_5819'); ?></label>
    ...
    <p>
      <label for="<?php echo esc_attr($this->get_field_id('email')); ?>"><?php _e('Email:', 'theme_5819'); ?></label>
...
function my_framework_widget_init() {
  register_widget('MY_Vcard_Widget');
}

add_action('widgets_init', 'my_framework_widget_init');